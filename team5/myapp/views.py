...
class Index(View):
    ...
    def get(self,request):
        courselist = Course.objects.all().order_by('course_no')[:10]
        return render(request, self.template_name, {'courselist': courselist})
...
class Detail(View):
    template_view = "myapp/detail.html"
    def get(self,request,course_no):
        course = get_object_or_404(Course, course_no=self.kwargs.get('course_no'))
        return render(request, self.template_view, {'course': course})
